...
> O núcleo foi movido da raiz para `packages/oinko` e renomeado de `@gba/ai-harness` para `@oinko/core`. A API `Agent` permanece compatível.
...
```typescript
import { Agent, builtinTools } from '@oinko/core';
...
```typescript
import { Agent, JevDecider, RecordingDecider, JsonlSink } from '@oinko/core';
