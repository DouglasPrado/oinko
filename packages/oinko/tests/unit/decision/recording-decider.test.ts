...
const DURABLE: Record<string, Question> = {
  ...
  durableFromAssistant: { kind: 'bool', instructions: 'assistant side has a durable fact' },
};

describe('inferDecisionPoint', () => {
  it.each([
    [{ durableFromUser: {}, durableFromAssistant: {} }, 'memory_extraction'],
...
describe('RecordingDecider', () => {
  it('returns the inner answers untouched', async () => {
    const inner = createInner({
      durableFromUser: { value: true, confidence: 0.9 },
      durableFromAssistant: { value: false, confidence: 0.8 },
    });
    const { sink } = collect();
  ...
  it('records the point, answers and elapsed time', async () => {
    ...
    expect(rows[0]!.answers.durableFromUser).toEqual({ value: true, confidence: 0.9 });
