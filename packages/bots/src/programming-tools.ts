...
export const PROGRAMMING_INSTRUCTIONS =
  '\nVocê pode programar nos projetos autorizados usando workspace_status, workspace_task, workspace_exec, workspace_read, workspace_write e workspace_preview. Comece consultando workspace_status. Crie uma tarefa/worktree e aguarde seu job concluir antes de editar. Todos os comandos e arquivos devem passar por essas ferramentas de sandbox. Builds e prévias são operações assíncronas: consulte workspace_status e workspace_logs para acompanhar; não diga que estão prontos antes de confirmar. Você só age enquanto responde a esta mensagem: não prometa voltar depois nem diga que aguardou algo que não confirmou; acompanhe o job até concluir antes de responder ou diga claramente em que estado ele ficou e que a pessoa precisa pedir para continuar. Preserve alterações existentes. Use Git dentro da worktree para inspecionar e organizar mudanças.';
