...
export class EnvironmentController {
  ...
  async handle(input: unknown): Promise<unknown> {
    ...
    return this.serial(project.id, async () => {
      ...
      const script = `const fs=require('node:fs'),path=require('node:path');let input='';process.stdin.on('data',x=>input+=x);process.stdin.on('end',()=>{const v=JSON.parse(input),root=fs.realpathSync(process.cwd()),p=path.resolve(root,v.path);const inside=x=>x===root||x.startsWith(root+path.sep);if(!inside(p))throw Error('Caminho fora da worktree');if(v.action==='writeFile'){const parts=path.relative(root,path.dirname(p)).split(path.sep).filter(Boolean);let parent=root;for(const part of parts){parent=path.join(parent,part);if(!fs.existsSync(parent))fs.mkdirSync(parent);if(!inside(fs.realpathSync(parent)))throw Error('Symlink fora da worktree');}if(fs.existsSync(p)&&!inside(fs.realpathSync(p)))throw Error('Symlink fora da worktree');fs.writeFileSync(p,v.content);console.log('Arquivo salvo');}else{if(!inside(fs.realpathSync(p)))throw Error('Symlink fora da worktree');const fd=fs.openSync(p,'r');try{const b=Buffer.alloc(200001),n=fs.readSync(fd,b,0,b.length,0);if(n>200000)throw Error('Arquivo excede 200 KB');process.stdout.write(b.subarray(0,n));}finally{fs.closeSync(fd);}}});`;
